...
{
    hipMemoryType mem_type = hipMemoryTypeHost;
    uint32_t is_managed = 0;
#define NUM_ATTRS 4
    hipPointer_attribute attr_type[NUM_ATTRS];
    void *attr_data[NUM_ATTRS];
    hipError_t result;

    attr_type[0] = HIP_POINTER_ATTRIBUTE_MEMORY_TYPE;
    attr_data[0] = &mem_type;
    attr_type[1] = HIP_POINTER_ATTRIBUTE_IS_MANAGED;
    attr_data[1] = &is_managed;
    attr_type[2] = HIP_POINTER_ATTRIBUTE_DEVICE_ORDINAL;
    attr_data[2] = &dev;
    attr_type[3] = HIP_POINTER_ATTRIBUTE_CONTEXT;
    attr_data[3] = &ctx;

    result = hipDrvPointerGetAttributes(4, attr_type, attr_data, (hipDeviceptr_t)address);

    is_dev = (mem_type == hipMemoryTypeDevice);

    return (hipSuccess != result);
}
...
int nixlUcxCudaCtx::cudaSetCtx()
{
    ...
    result = hipCtxSetCurrent(pthrCudaCtx);
