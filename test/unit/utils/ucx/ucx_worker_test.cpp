...
#ifdef USE_VRAM
    checkCudaError(hipSetDevice(gpu_id), "Failed to set device");
    checkCudaError(hipMalloc(&buffer[0], buf_size), "Failed to allocate CUDA buffer 0");
    checkCudaError(hipMalloc(&buffer[1], buf_size), "Failed to allocate CUDA buffer 1");
#else
...
#ifdef USE_VRAM
    checkCudaError(hipMemset(buffer[1], 0xbb, buf_size), "Failed to memset");
    checkCudaError(hipMemset(buffer[0], 0xda, buf_size), "Failed to memset");
#else
...
#ifdef USE_VRAM
    checkCudaError(hipMemcpy(chk_buffer, buffer[1], 128, hipMemcpyDeviceToHost), "Failed to memcpy");
#else
...
#ifdef USE_VRAM
    checkCudaError(hipMemset(buffer[0], 0xbb, buf_size), "Failed to memset");
    checkCudaError(hipMemset(buffer[1], 0xbb, buf_size/3), "Failed to memset");
    checkCudaError(hipMemset(buffer[1] + 32, 0xda, buf_size - buf_size / 3), "Failed to memset");
#else
...
#ifdef USE_VRAM
    checkCudaError(hipMemcpy(chk_buffer, buffer[0], buf_size, hipMemcpyDeviceToHost), "Failed to memcpy");
#else
...
#ifdef USE_VRAM
    checkCudaError(hipFree(buffer[0]), "Failed to allocate CUDA buffer 0");
    checkCudaError(hipFree(buffer[1]), "Failed to allocate CUDA buffer 0");
#else
    free(buffer[0]);
    free(buffer[1]);
#endif
